...
# TODO make this more robust later (if script is called from another location?)
INGEN_ROOT_DIR="$PWD"
...
INGEN_PACKAGE_SPEC_YAML_FILE="${INGEN_ROOT_DIR}/ingen_package_spec.yml"
INGEN_PACKAGE_UPDATES_YAML_FILE="${INGEN_ROOT_DIR}/ingen_package_updates.yml"
INGEN_PACKAGE_CURRENT_YAML_FILE="${INGEN_ROOT_DIR}/ingen_package_current.yml"
INGEN_INSTALLER_SCRIPT_TEMPLATE_FILE="${INGEN_ROOT_DIR}/ingen_installer_script_template.sh"
...
INGEN_BUILDER_DATA_CONDA_PACKAGES_JSON_FILE="$INGEN_BUILDER_DATA_DIR/conda_list.json"
INGEN_BUILDER_DATA_PIP_PACKAGES_JSON_FILE="$INGEN_BUILDER_DATA_DIR/pip_list.json"
...
function ingen_cleanup() {
    ...
    if [ "$DEVELOPMENT_MODE_ENABLED" == "$FALSE_VAL" ] ; then
        ...
        if [ -d "$INGEN_BUILDER_DATA_DIR" ] ; then
            ...
            # clean it up
            echo "clean up builder data in:" "$INGEN_BUILDER_DATA_DIR"
    ...
    if [ "$DEVELOPMENT_MODE_ENABLED" == "$FALSE_VAL" ] ; then
        ...
        if [ -d "$INGEN_CONDA_INSTALL_DIR" ] ; then
            ...
            # clean it up
            echo "clean up existing conda installation in:" "$INGEN_CONDA_INSTALL_DIR"
...
if [ "$DEVELOPMENT_MODE_ENABLED" == "$TRUE_VAL" ] ; then
    ...
    if [ -d "$INGEN_CONDA_INSTALL_DIR" ] ; then
        ...
        echo "[DEV MODE] reusing existing conda install in:" "$INGEN_CONDA_INSTALL_DIR"
        ...
        echo "[DEV MODE] conda installation does not exist, create new conda install"
    ...
    if [ -d "$INGEN_CONDA_INSTALL_DIR" ] ; then
        ...
        # clean it up
        echo "clean up existing conda env in:" "$INGEN_CONDA_INSTALL_DIR"
        rm -rf "$INGEN_CONDA_INSTALL_DIR"
...
if [ "$DEVELOPMENT_MODE_ENABLED" == "$TRUE_VAL" ] ; then
    ...
    if [ -d "$INGEN_CONDA_ENV__INGEN__ENV_PREFIX" ] ; then
        ...
        # already exists, nothing do further to setup a new env in development mode
        echo "[DEV MODE] reusing existing conda env in:" "$INGEN_CONDA_ENV__INGEN__ENV_PREFIX"
    ...
    if [ -d "$INGEN_CONDA_ENV__INGEN__ENV_PREFIX" ] ; then
        ...
        # clean it up
        echo "clean up existing conda env in:" "$INGEN_CONDA_ENV__INGEN__ENV_PREFIX"
...
if [ ! $? == 0 ] ; then
    ...
    echo
    echo "ERROR: test_local_conda_env failed, aborting."
    exit 1
...
python3 ./ingen_generate_conda_env_configuration.py \
            --ingen_package_yaml_file="$INGEN_PACKAGE_SPEC_YAML_FILE" \
...
conda list --json > "$INGEN_BUILDER_DATA_CONDA_PACKAGES_JSON_FILE"

# grab the pip packages information
pip list --format=json > "$INGEN_BUILDER_DATA_PIP_PACKAGES_JSON_FILE"
...
python3 ./ingen_generate_package_updates.py \
            --package_spec_yaml_file="$INGEN_PACKAGE_SPEC_YAML_FILE" \
            --package_updates_yaml_file="$INGEN_PACKAGE_UPDATES_YAML_FILE" \
            --conda_packages_json_file="$INGEN_BUILDER_DATA_CONDA_PACKAGES_JSON_FILE" \
            --pip_packages_json_file="$INGEN_BUILDER_DATA_PIP_PACKAGES_JSON_FILE"


# check if any updates available that we want? no(0) -> exit, yes(1) -> proceed.
python3 ./ingen_check_for_updates.py \
            --package_current_yaml_file="$INGEN_PACKAGE_CURRENT_YAML_FILE" \
            --package_updates_yaml_file="$INGEN_PACKAGE_UPDATES_YAML_FILE" \
...
if [ $PACKAGE_UPDATE_AVAILABLE == 0 ] ; then
    ...
    echo
    echo "No new updates are available for the Symbiflow Packages"
    echo "Exiting Ingen"
    ingen_cleanup
    exit 0
...
python3 ./ingen_generate_conda_env_configuration.py \
            --ingen_package_yaml_file="$INGEN_PACKAGE_UPDATES_YAML_FILE" \
            --conda_env_config_dir="$INGEN_SYMBIFLOW_INSTALLER_DIR" \


# copy the package updates yaml file into the symbiflow_installer dir
cp -v "$INGEN_PACKAGE_UPDATES_YAML_FILE" "$INGEN_SYMBIFLOW_INSTALLER_PACKAGE_UPDATES_YAML"
...
python3 ./ingen_generate_installer_script.py \
            --ingen_package_yaml_file="$INGEN_PACKAGE_UPDATES_YAML_FILE" \
            --ingen_installer_script_template="$INGEN_INSTALLER_SCRIPT_TEMPLATE_FILE" \
...
##########################################################################################
# STEP 8 : publish the self-extracting symbiflow package installer
##########################################################################################
...
##########################################################################################
# STEP 9 : run the self-extracting symbiflow package installer through full CI
##########################################################################################
